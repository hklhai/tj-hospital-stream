...
PID=`ps -ef|grep SyncMediumVoltageMySQL2Db2|grep -v grep|awk '{print $2}'`
echo $PID
if [ $PID ];then
	echo "Wait!"
	#	echo "Kill Thread!"
	#	kill -9 $PID
	#	nohup /root/app/flink-1.8.0/bin/flink run -c com.hxqh.sync.SyncMediumVoltageMySQL2Db2 /root/TJJar/sync/tj-hospital.jar > /root/shell/SyncMediumVoltageMySQL2Db2.log 2>&1 &
else
	echo "Start Task!"
	nohup /root/app/flink-1.8.0/bin/flink run -c com.hxqh.sync.SyncMediumVoltageMySQL2Db2 /root/TJJar/sync/tj-hospital.jar > /root/shell/SyncMediumVoltageMySQL2Db2.log 2>&1 &
fi
