...
import com.hxqh.task.alarm.capacitor.CapacitorFirstAlarm;
import com.hxqh.task.alarm.capacitor.CapacitorSecondAlarm;
import com.hxqh.task.alarm.capacitor.CapacitorThirdAlarm;
import com.hxqh.transfer.LowPressureCapacitorWaterEmitter;
...
public class LowPressureCapacitorTask {

    public static void main(String[] args) {
        args = new String[]{"--input-topic", "lowcapacitor", "--bootstrap.servers", "tj-hospital.com:9092",
        ...
        ).inAppendMode().registerTableSource("capacitor");

        Table table = tableEnvironment.sqlQuery("select * from capacitor");
        DataStream<Row> data = tableEnvironment.toAppendStream(table, Row.class);
        data.assignTimestampsAndWatermarks(new LowPressureCapacitorWaterEmitter());

        DataStream<Yx> firstAlarm = data.flatMap(new CapacitorFirstAlarm());
        DataStream<Yx> secondAlarm = data.flatMap(new CapacitorSecondAlarm());
        DataStream<Yx> thridAlarm = data.flatMap(new CapacitorThirdAlarm());
        ...
        try {
            env.execute("LowPressureCapacitorTask");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
