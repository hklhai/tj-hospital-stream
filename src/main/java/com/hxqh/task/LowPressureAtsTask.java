...
import com.hxqh.task.alarm.ats.AtsFirstAlarm;
import com.hxqh.task.alarm.ats.AtsSecondAlarm;
import com.hxqh.task.alarm.ats.AtsThirdAlarm;
import com.hxqh.transfer.LowPressureAtsWaterEmitter;
...
public class LowPressureAtsTask {

    public static void main(String[] args) {
        args = new String[]{"--input-topic", "lowats", "--bootstrap.servers", "tj-hospital.com:9092",
                "--zookeeper.connect", "tj-hospital.com:2181", "--group.id", "yctest", "--output-topic", "mediumvoltage"};
        ...
        ).inAppendMode().registerTableSource("ats");

        Table table = tableEnvironment.sqlQuery("select * from ats");
        DataStream<Row> data = tableEnvironment.toAppendStream(table, Row.class);
        data.assignTimestampsAndWatermarks(new LowPressureAtsWaterEmitter());

        DataStream<Yx> firstAlarm = data.flatMap(new AtsFirstAlarm());
        DataStream<Yx> secondAlarm = data.flatMap(new AtsSecondAlarm());
        DataStream<Yx> thridAlarm = data.flatMap(new AtsThirdAlarm());
        ...
        try {
            env.execute("LowPressureAtsTask");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
