...
public class LowPressureCurrentLoadYear {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String maxQuery = "select u.* from (SELECT IEDNAME,ASSETYPE,LOCATION,PRODUCTMODELC,PRODUCTMODELB,CREATETIME,PHASECURRENTAVG,row_number() over(PARTITION BY IEDNAME ORDER BY PHASECURRENTAVG desc) AS rn FROM RE_LP_CURRENT_MONTH where CREATETIME like '" + RemindDateUtils.getLastYear() + "%' ) u where u.rn = 1";
        ...
        String minQuery = "select u.* from (SELECT IEDNAME,ASSETYPE,LOCATION,PRODUCTMODELC,PRODUCTMODELB,CREATETIME,PHASECURRENTAVG,row_number() over(PARTITION BY IEDNAME ORDER BY PHASECURRENTAVG asc) AS rn FROM RE_LP_CURRENT_MONTH where CREATETIME like '" + RemindDateUtils.getLastYear() + "%' ) u where u.rn = 1";
        ...
        String insertQuery = "INSERT INTO RE_LP_CURRENT_LOAD_Y (IEDNAME,ASSETYPE,LOCATION,PRODUCTMODELC,PRODUCTMODELB,MINMONTH,MAXMONTH,CREATETIME)" +
                " VALUES(?,?,?,?,?,?,?,?)";
        ...
        env.execute("LowPressureCurrentLoadYear");
    }
