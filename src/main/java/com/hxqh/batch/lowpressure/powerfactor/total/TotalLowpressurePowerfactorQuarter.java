...
public class TotalLowpressurePowerfactorQuarter {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String transQuery = "select ASSETYPE,LOCATION,PRODUCTMODELC,POWERFACTORAVG,CREATETIME from RE_LP_POWERFACTOR_M where ASSETYPE like '低压开关设备%' and CREATETIME in " + RemindDateUtils.getLastQuarterString();
        ...
        String insertQuery = "INSERT INTO  RE_ALL_LP_POWERFACTOR_Q(ASSETYPE,LOCATION,PRODUCTMODELC,CREATETIME,MINMONTHSTR,MAXMONTHSTR) VALUES(?,?,?,?,?,?)";
        JDBCOutputFormat.JDBCOutputFormatBuilder outputBuilder =
                JDBCOutputFormat.buildJDBCOutputFormat().setDrivername(DB2_DRIVER_NAME).setDBUrl(DB2_DB_URL)
                        .setQuery(insertQuery).setSqlTypes(type).setUsername(DB2_USERNAME).setPassword(DB2_PASSWORD);
        result.output(outputBuilder.finish());

        env.execute("TotalLowpressurePowerfactorQuarter");
    }
