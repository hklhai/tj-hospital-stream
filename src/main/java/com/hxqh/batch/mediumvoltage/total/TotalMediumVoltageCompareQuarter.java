...
public class TotalMediumVoltageCompareQuarter {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String quarterQuery = "select ASSETYPE,PRODUCTMODEL,LOCATION,SCORE,CREATETIME from RE_TOTAL_SCORE_QUARTER  where ASSETYPE='中压开关设备' and CREATETIME in " + RemindDateUtils.getLastTwoQuarterString();
        JDBCInputFormat.JDBCInputFormatBuilder quarterBuilder =
                JDBCInputFormat.buildJDBCInputFormat().setDrivername(DB2_DRIVER_NAME).setDBUrl(DB2_DB_URL)
                        ...
                        new TypeInformation<?>[]{BasicTypeInfo.STRING_TYPE_INFO, BasicTypeInfo.STRING_TYPE_INFO, BasicTypeInfo.STRING_TYPE_INFO, BasicTypeInfo.BIG_DEC_TYPE_INFO, BasicTypeInfo.STRING_TYPE_INFO
        ...
        String insertQuery = "INSERT INTO RE_TOTAL_COMPARE_QUARTER(ASSETYPE,PRODUCTMODEL,LOCATION,SCORE,CREATETIME,COMPARISON,RATIO) VALUES(?,?,?,?,?,?,?)";
        JDBCOutputFormat.JDBCOutputFormatBuilder outputBuilder =
                JDBCOutputFormat.buildJDBCOutputFormat().setDrivername(DB2_DRIVER_NAME).setDBUrl(DB2_DB_URL)
                        .setQuery(insertQuery).setSqlTypes(type).setUsername(DB2_USERNAME).setPassword(DB2_PASSWORD);
        result.output(outputBuilder.finish());

        env.execute("TotalMediumVoltageCompareQuarter");
    }
