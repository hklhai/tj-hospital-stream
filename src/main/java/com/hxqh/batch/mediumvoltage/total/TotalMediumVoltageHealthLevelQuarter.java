...
public class TotalMediumVoltageHealthLevelQuarter {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String quarterQuery = "select ASSETYPE,PRODUCTMODEL,LOCATION,SCORE from RE_SCORE_QUARTER " +
        ...
        String insertQuery = "INSERT INTO RE_TOTAL_HEALTH_QUARTER(ASSETYPE,PRODUCTMODEL,LOCATION,excellent,good,range,CREATETIME) VALUES(?,?,?,?,?,?,?)";
        JDBCOutputFormat.JDBCOutputFormatBuilder outputBuilder =
                JDBCOutputFormat.buildJDBCOutputFormat().setDrivername(DB2_DRIVER_NAME).setDBUrl(DB2_DB_URL)
                        .setQuery(insertQuery).setSqlTypes(type).setUsername(DB2_USERNAME).setPassword(DB2_PASSWORD);
        sink.output(outputBuilder.finish());

        env.execute("TotalMediumVoltageHealthLevelQuarter");
    }
