...
public class TotalMediumVoltageScoreYear {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String quarterQuery = "select ASSETYPE,PRODUCTMODEL,LOCATION,avg(SCORE) as SCORE from RE_TOTAL_SCORE_QUARTER " +
                "where ASSETYPE='中压开关设备' and CREATETIME like '" + RemindDateUtils.getLastYear() + "%' group by ASSETYPE,PRODUCTMODEL,LOCATION";
        ...
        String insertQuery = "INSERT INTO RE_TOTAL_SCORE_YEAR(ASSETYPE,PRODUCTMODEL,LOCATION,SCORE,CREATETIME) VALUES(?,?,?,?,?)";
        JDBCOutputFormat.JDBCOutputFormatBuilder outputBuilder =
                JDBCOutputFormat.buildJDBCOutputFormat().setDrivername(DB2_DRIVER_NAME).setDBUrl(DB2_DB_URL)
                        .setQuery(insertQuery).setSqlTypes(type).setUsername(DB2_USERNAME).setPassword(DB2_PASSWORD);
        sinkRow.output(outputBuilder.finish());

        env.execute("TotalMediumVoltageScoreYear");
    }
