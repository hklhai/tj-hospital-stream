...
public class MediumVoltageUseEfficiencyCompareYear {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String quarterQuery = "select IEDNAME,ASSETYPE,RUNNINGTIME,CREATETIME,PRODUCTMODEL,LOCATION,UTILIZATIONRATIO from RE_VOLTAGE_UE_YEAR where ASSETYPE='中压开关设备' and CREATETIME in " + RemindDateUtils.getLastTwoYearString();
        JDBCInputFormat.JDBCInputFormatBuilder quarterBuilder =
                JDBCInputFormat.buildJDBCInputFormat().setDrivername(DB2_DRIVER_NAME).setDBUrl(DB2_DB_URL)
                        ...
                        new TypeInformation<?>[]{BasicTypeInfo.STRING_TYPE_INFO, BasicTypeInfo.STRING_TYPE_INFO, BasicTypeInfo.DOUBLE_TYPE_INFO, BasicTypeInfo.STRING_TYPE_INFO, BasicTypeInfo.STRING_TYPE_INFO, BasicTypeInfo.STRING_TYPE_INFO, BasicTypeInfo.DOUBLE_TYPE_INFO
        ...
        DataSet<Tuple9<String, String, Double, String, String, String, Double, String, Double>> quarter = quarterRow.map(new MapFunction<Row, Tuple9<String, String, Double, String, String, String, Double, String, Double>>() {
            ...
            public Tuple9<String, String, Double, String, String, String, Double, String, Double> map(Row row) throws Exception {
                return Tuple9.of(row.getField(0).toString(), row.getField(1).toString(),
                        Double.parseDouble(row.getField(2).toString()), row.getField(3).toString(), row.getField(4).toString(), row.getField(5).toString(),
                        Double.parseDouble(row.getField(6).toString()), "", 0.0);
            }
        });
        ...
        String insertQuery = "INSERT INTO RE_VOLTAGE_UE_COMP_YEAR (IEDNAME,ASSETYPE,RUNNINGTIME,CREATETIME,PRODUCTMODEL,LOCATION,UTILIZATIONRATIO,COMPARISON,RATIO) VALUES(?,?,?,?,?,?,?,?,?)";
        JDBCOutputFormat.JDBCOutputFormatBuilder outputBuilder =
                JDBCOutputFormat.buildJDBCOutputFormat().setDrivername(DB2_DRIVER_NAME).setDBUrl(DB2_DB_URL)
                        .setQuery(insertQuery).setSqlTypes(type).setUsername(DB2_USERNAME).setPassword(DB2_PASSWORD);
        result.output(outputBuilder.finish());

        env.execute("MediumVoltageUseEfficiencyCompareYear");
    }
