...
public class TotalMediumVoltageUseEfficiencyLevelYear {

    public static void main(String[] args) throws Exception {
        final int[] type = getType();

        ExecutionEnvironment env = ExecutionEnvironment.getExecutionEnvironment();
        String quarterQuery = "select ASSETYPE,PRODUCTMODEL,LOCATION,UTILIZATIONRATIO from RE_VOLTAGE_UE_YEAR " +
        ...
        String insertQuery = "INSERT INTO RE_ALL_VOL_UE_HEALTH_YEAR(ASSETYPE,PRODUCTMODEL,LOCATION,excellent,good,range,CREATETIME) VALUES(?,?,?,?,?,?,?)";
        JDBCOutputFormat.JDBCOutputFormatBuilder outputBuilder =
                JDBCOutputFormat.buildJDBCOutputFormat().setDrivername(DB2_DRIVER_NAME).setDBUrl(DB2_DB_URL)
                        .setQuery(insertQuery).setSqlTypes(type).setUsername(DB2_USERNAME).setPassword(DB2_PASSWORD);
        sink.output(outputBuilder.finish());

        env.execute("TotalMediumVoltageUseEfficiencyLevelYear");
    }
