...
public class TotalMediumVoltageUseEfficiencyCompareQuarter {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String quarterQuery = "select ASSETYPE,PRODUCTMODEL,LOCATION,AVGUTILIZATIONRATIO,TOTALRUNNINGTIME,CREATETIME " +
                "from RE_ALL_VOLTAGE_UE_QUARTER where ASSETYPE='中压开关设备' and CREATETIME in " + RemindDateUtils.getLastTwoQuarterString();
        ...
        DataSet<Tuple6<String, Double, Double, String, String, Double>> quarter = quarterRow.map(new MapFunction<Row, Tuple6<String, Double, Double, String, String, Double>>() {
            @Override
            public Tuple6<String, Double, Double, String, String, Double> map(Row row) throws Exception {
                ...
                return Tuple6.of(key, Double.parseDouble(row.getField(3).toString()),
                        Double.parseDouble(row.getField(4).toString()), row.getField(5).toString(), "", 0.0);
            }
        });


        DataSet<Tuple6<String, Double, Double, String, String, Double>> reduce = quarter.groupBy(0).reduce(new ReduceFunction<Tuple6<String, Double, Double, String, String, Double>>() {
            @Override
        ...
        String insertQuery = "INSERT INTO RE_ALL_VOLTAGE_UE_COMP_QUARTER (ASSETYPE,PRODUCTMODEL,LOCATION,AVGUTILIZATIONRATIO,TOTALRUNNINGTIME,CREATETIME,COMPARISON,RATIO) VALUES(?,?,?,?,?,?,?,?)";
        JDBCOutputFormat.JDBCOutputFormatBuilder outputBuilder =
                JDBCOutputFormat.buildJDBCOutputFormat().setDrivername(DB2_DRIVER_NAME).setDBUrl(DB2_DB_URL)
                        .setQuery(insertQuery).setSqlTypes(type).setUsername(DB2_USERNAME).setPassword(DB2_PASSWORD);
        result.output(outputBuilder.finish());

        env.execute("TotalMediumVoltageUseEfficiencyCompareQuarter");
    }

    private static int[] getType() {
        return new int[]{Types.VARCHAR, Types.VARCHAR, Types.VARCHAR, Types.DOUBLE, Types.DOUBLE, Types.VARCHAR, Types.VARCHAR, Types.DOUBLE
        };
    }
}
