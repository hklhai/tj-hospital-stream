...
public class TotalMediumVoltageLoadFactorCompareYear {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String quarterQuery = "select ASSETYPE,PRODUCTMODEL,LOCATION,LOADFACTOR,CREATETIME from RE_TOTAL_LOAD_YEAR where CREATETIME in " + RemindDateUtils.getLastTwoYearString();
        ...
        DataSet<Tuple5<String, Double, String, String, Double>> quarter = quarterRow.map(new MapFunction<Row, Tuple5<String, Double, String, String, Double>>() {
            ...
            public Tuple5<String, Double, String, String, Double> map(Row row) throws Exception {
                ...
                return Tuple5.of(key, Double.parseDouble(row.getField(3).toString()), row.getField(4).toString(), "", 0.0);
            }
        });
        quarter.print();
        ...
        String insertQuery = "INSERT INTO RE_TOTAL_LOAD_COMPARE_YEAR(ASSETYPE,PRODUCTMODEL,LOCATION,LOADFACTOR,CREATETIME,COMPARISON,RATIO) VALUES(?,?,?,?,?,?,?)";
        JDBCOutputFormat.JDBCOutputFormatBuilder outputBuilder =
                JDBCOutputFormat.buildJDBCOutputFormat().setDrivername(DB2_DRIVER_NAME).setDBUrl(DB2_DB_URL)
                        .setQuery(insertQuery).setSqlTypes(type).setUsername(DB2_USERNAME).setPassword(DB2_PASSWORD);
        result.output(outputBuilder.finish());

        env.execute("TotalMediumVoltageLoadFactorCompareQuarter");
    }
