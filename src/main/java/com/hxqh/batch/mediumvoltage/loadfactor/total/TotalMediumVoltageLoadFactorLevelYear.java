...
public class TotalMediumVoltageLoadFactorLevelYear {

    public static void main(String[] args) throws Exception {
        final int[] type = getType();

        ExecutionEnvironment env = ExecutionEnvironment.getExecutionEnvironment();
        String quarterQuery = "select ASSETYPE,PRODUCTMODEL,LOCATION,LOADFACTOR from RE_LOAD_YEAR " +
        ...
        DataSet<Tuple2<String, Integer>> levelTuple = quarterRow.map(new MapFunction<Row, Tuple2<String, Integer>>() {
            ...
            public Tuple2<String, Integer> map(Row row) throws Exception {
                ...
                String key = row.getField(0).toString() + "|" + row.getField(1).toString() + "|" +
                        row.getField(2).toString() + "|" + LevelUtils.computeLoadFactorLevel(score);
        ...
        String insertQuery = "INSERT INTO RE_TOTAL_LOAD_HEALTH_YEAR(ASSETYPE,PRODUCTMODEL,LOCATION,excellent,good,range,CREATETIME) VALUES(?,?,?,?,?,?,?)";
        JDBCOutputFormat.JDBCOutputFormatBuilder outputBuilder =
                JDBCOutputFormat.buildJDBCOutputFormat().setDrivername(DB2_DRIVER_NAME).setDBUrl(DB2_DB_URL)
                        .setQuery(insertQuery).setSqlTypes(type).setUsername(DB2_USERNAME).setPassword(DB2_PASSWORD);
        sink.output(outputBuilder.finish());

        env.execute("TotalMediumVoltageLoadFactorLevelYear");
    }
