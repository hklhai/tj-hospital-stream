...
public class TotalMediumVoltageElectricalMeasurementCompYear {

    public static void main(String[] args) throws Exception {
        final int[] type = getType();
        ExecutionEnvironment env = ExecutionEnvironment.getExecutionEnvironment();

        String quarterQuery = "select ASSETYPE,PRODUCTMODEL,LOCATION,PRODUCTMODELC,CREATETIME,avg(ElectricDegree) as ElectricDegree from RE_ALL_VOLTAGE_EM_YEAR " +
                "where ASSETYPE='中压开关设备' and CREATETIME in " + RemindDateUtils.getLastTwoYearString() + " group by ASSETYPE,PRODUCTMODEL,LOCATION,PRODUCTMODELC,CREATETIME";
        ...
        String insertQuery = "INSERT INTO RE_ALL_VOL_EM_COMP_YEAR(ASSETYPE,PRODUCTMODEL,LOCATION,PRODUCTMODELC,ELECTRICDEGREE,CREATETIME,COMPARISON,RATIO) VALUES(?,?,?,?,?,?,?,?)";
        JDBCOutputFormat.JDBCOutputFormatBuilder outputBuilder =
                JDBCOutputFormat.buildJDBCOutputFormat().setDrivername(DB2_DRIVER_NAME).setDBUrl(DB2_DB_URL)
                        .setQuery(insertQuery).setSqlTypes(type).setUsername(DB2_USERNAME).setPassword(DB2_PASSWORD);
        result.output(outputBuilder.finish());

        env.execute("TotalMediumVoltageElectricalMeasurementCompYear");
    }
