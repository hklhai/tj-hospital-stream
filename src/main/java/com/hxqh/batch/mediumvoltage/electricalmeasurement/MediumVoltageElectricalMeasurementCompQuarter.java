...
public class MediumVoltageElectricalMeasurementCompQuarter {

    public static void main(String[] args) throws Exception {
        final int[] type = getType();
        ExecutionEnvironment env = ExecutionEnvironment.getExecutionEnvironment();

        String quarterQuery = "select IEDNAME,ASSETYPE,PRODUCTMODEL,LOCATION,PRODUCTMODELC,ElectricDegree,CREATETIME from RE_VOLTAGE_EM_QUARTER where ASSETYPE='中压开关设备' and CREATETIME in " + RemindDateUtils.getLastTwoQuarterString();
        ...
        String insertQuery = "INSERT INTO RE_VOLTAGE_EM_COMP_QUARTER (IEDNAME,ASSETYPE,PRODUCTMODEL,LOCATION,PRODUCTMODELC,ELECTRICDEGREE,CREATETIME,COMPARISON,RATIO) VALUES(?,?,?,?,?,?,?,?,?)";
        JDBCOutputFormat.JDBCOutputFormatBuilder outputBuilder =
                JDBCOutputFormat.buildJDBCOutputFormat().setDrivername(DB2_DRIVER_NAME).setDBUrl(DB2_DB_URL)
                        .setQuery(insertQuery).setSqlTypes(type).setUsername(DB2_USERNAME).setPassword(DB2_PASSWORD);
        result.output(outputBuilder.finish());

        env.execute("MediumVoltageElectricalMeasurementCompQuarter");
    }
