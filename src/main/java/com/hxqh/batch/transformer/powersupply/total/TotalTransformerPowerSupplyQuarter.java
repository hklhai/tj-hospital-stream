...
/**
 * 整体变压器供用电状况--季度（按照productmodelC分组）
 * <p>
...
public class TotalTransformerPowerSupplyQuarter {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String selectQuery = "select ASSETYPE,PRODUCTMODELC,avg(LOADVALUE),avg(LOADRATIO),avg(LOSSVALUE),avg(LOSSRATIO) " +
                "from RE_TRANS_PS_QUARTER where ASSETYPE='变压器' and CREATETIME = '" + RemindDateUtils.getLastQuarter() + "' group by ASSETYPE,PRODUCTMODELC";
        ...
        String insertQuery = "INSERT INTO RE_ALL_TRANS_PS_QUARTER (ASSETYPE,PRODUCTMODELC,LOADVALUE,LOADRATIO,LOSSVALUE,LOSSRATIO,CREATETIME) VALUES(?,?,?,?,?,?,?)";
        JDBCOutputFormat.JDBCOutputFormatBuilder outputBuilder =
                JDBCOutputFormat.buildJDBCOutputFormat().setDrivername(DB2_DRIVER_NAME).setDBUrl(DB2_DB_URL)
                        .setQuery(insertQuery).setSqlTypes(type).setUsername(DB2_USERNAME).setPassword(DB2_PASSWORD);
        result.output(outputBuilder.finish());

        env.execute("TotalTransformerPowerSupplyQuarter");
    }
