...
public class TotalTransformerPowerSupplyCompareYear {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String selectQuery = "select ASSETYPE,PRODUCTMODELC,LOADVALUE,LOADRATIO,LOSSVALUE,LOSSRATIO,CREATETIME " +
                "from RE_ALL_TRANS_PS_YEAR where ASSETYPE='变压器' and CREATETIME in " + RemindDateUtils.getLastTwoYearString() + " ";
        ...
        String insertQuery = "INSERT INTO RE_ALL_TRANS_PS_COM_YEAR (ASSETYPE,PRODUCTMODELC,LOADVALUE,LOADRATIO,LOSSVALUE,LOSSRATIO,COMPARISON,RATIO,CREATETIME) VALUES(?,?,?,?,?,?,?,?,?)";
        JDBCOutputFormat.JDBCOutputFormatBuilder outputBuilder =
                JDBCOutputFormat.buildJDBCOutputFormat().setDrivername(DB2_DRIVER_NAME).setDBUrl(DB2_DB_URL)
                        .setQuery(insertQuery).setSqlTypes(type).setUsername(DB2_USERNAME).setPassword(DB2_PASSWORD);
        result.output(outputBuilder.finish());

        env.execute("TotalTransformerPowerSupplyCompareYear");
    }
