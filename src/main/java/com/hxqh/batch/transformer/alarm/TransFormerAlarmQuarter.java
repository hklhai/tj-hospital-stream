package com.hxqh.batch.transformer.alarm;
...
public class TransFormerAlarmQuarter {
...
//        String sqlFirst = "select IEDName,alarmLevel,count(1) as levelCount from yx1 where Val='1' and assetYpe = '变压器' and ColTime>='2019-03-01 00:00:00' and ColTime<='2020-04-30 23:59:59'  group by IEDName,alarmLevel";
        String sqlFirst = "select IEDName,alarmLevel,count(*) as levelCount from yx1 where Val='1' and assetYpe = '变压器' and ColTime>='" + start + "' and ColTime<='" + end + "'  group by IEDName,alarmLevel";
        ...
        String insertQuery = "INSERT INTO  RE_TRANS_ALARM_QUARTER(IEDNAME,ALARMLEVEL1COUNT,ALARMLEVEL1RATIO,ALARMLEVEL2COUNT,ALARMLEVEL2RATIO,ALARMLEVEL3COUNT,ALARMLEVEL3RATIO,VARIABLENAMEMAX,CREATETIME) VALUES(?,?,?,?,?,?,?,?,?)";
        JDBCOutputFormat.JDBCOutputFormatBuilder outputBuilder =
                JDBCOutputFormat.buildJDBCOutputFormat().setDrivername(DB2_DRIVER_NAME).setDBUrl(DB2_DB_URL)
                        .setQuery(insertQuery).setSqlTypes(type).setUsername(DB2_USERNAME).setPassword(DB2_PASSWORD);
        result.output(outputBuilder.finish());

        env.execute("TransFormerAlarmQuarter");
