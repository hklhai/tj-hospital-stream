...
public class TransformerUseEfficiencyYear {
    ...
    public static void main(String[] args) throws Exception {
        ...
        //中压设备  like '2020%'才有数据
        String mysqlQuery = "select IEDNAME,sum(RUNNINGTIME) AS TOTALRUNNINGTIME,sum(DOWNTIME) AS TOTALDOWNTIME FROM yc_medium_voltage_run_month " +
                "where PARTICULARTIME like '" + RemindDateUtils.getLastYear() + "%'  group by IEDNAME";
        ...
        String insertQuery = "INSERT INTO RE_TRANS_UE_YEAR(IEDNAME,ASSETYPE,PRODUCTMODEL,LOCATION,PRODUCTMODELB,PRODUCTMODELC,PARENT,RUNNINGTIME,UTILIZATIONRATIO,DOWNTIME,CREATETIME) VALUES(?,?,?,?,?,?,?,?,?,?,?)";
        JDBCOutputFormat.JDBCOutputFormatBuilder outputBuilder =
                JDBCOutputFormat.buildJDBCOutputFormat().setDrivername(DB2_DRIVER_NAME).setDBUrl(DB2_DB_URL)
                        .setQuery(insertQuery).setSqlTypes(type).setUsername(DB2_USERNAME).setPassword(DB2_PASSWORD);
        result.output(outputBuilder.finish());

        env.execute("TransformerUseEfficiencyYear");
    }
