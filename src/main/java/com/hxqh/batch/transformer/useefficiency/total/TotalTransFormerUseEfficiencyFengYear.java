 ...
 * <p>
 */
public class TotalTransFormerUseEfficiencyFengYear {
...
//        String end = "2020-05-30 23:59:59";
        ...
        String insertQuery = "INSERT INTO RE_ALL_TRANS_UE_FENG_Y(ASSETYPE,LOCATION,PRODUCTMODEL,PRODUCTMODELB,PRODUCTMODELC,RUNNINGTIME,CREATETIME) VALUES(?,?,?,?,?,?,?)";
        JDBCOutputFormat.JDBCOutputFormatBuilder outputBuilder =
                JDBCOutputFormat.buildJDBCOutputFormat().setDrivername(DB2_DRIVER_NAME).setDBUrl(DB2_DB_URL)
                        .setQuery(insertQuery).setSqlTypes(type).setUsername(DB2_USERNAME).setPassword(DB2_PASSWORD);
        result.output(outputBuilder.finish());

        env.execute("TotalTransFormerUseEfficiencyFengYear");
    }
