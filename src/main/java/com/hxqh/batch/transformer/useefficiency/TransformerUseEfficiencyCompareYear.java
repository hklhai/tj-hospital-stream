...
public class TransformerUseEfficiencyCompareYear {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String transQuery = "select IEDNAME,ASSETYPE,PRODUCTMODEL,LOCATION,PRODUCTMODELB,PRODUCTMODELC,PARENT," +
                "UTILIZATIONRATIO,DOWNTIME,RUNNINGTIME,CREATETIME from RE_TRANS_UE_YEAR " +
                "where ASSETYPE='变压器' and CREATETIME in "  + RemindDateUtils.getLastTwoYearString();
        ...
        result.print();
        String insertQuery = "INSERT INTO RE_TRANS_UE_COM_YEAR(IEDNAME,ASSETYPE,PRODUCTMODEL,LOCATION,PRODUCTMODELB,PRODUCTMODELC,PARENT,UTILIZATIONRATIO,DOWNTIME,RUNNINGTIME,CREATETIME,COMPARISON,RATIO) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)";
        JDBCOutputFormat.JDBCOutputFormatBuilder outputBuilder =
                JDBCOutputFormat.buildJDBCOutputFormat().setDrivername(DB2_DRIVER_NAME).setDBUrl(DB2_DB_URL)
                        .setQuery(insertQuery).setSqlTypes(type).setUsername(DB2_USERNAME).setPassword(DB2_PASSWORD);
        result.output(outputBuilder.finish());

        env.execute("TransformerUseEfficiencyCompareYear");
    }
