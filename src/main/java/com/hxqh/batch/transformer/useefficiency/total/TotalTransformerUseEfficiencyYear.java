...
public class TotalTransformerUseEfficiencyYear {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String transQuery = "select ASSETYPE,PRODUCTMODELC,sum(RUNNINGTIME),sum(UTILIZATIONRATIO)/count(*),sum(DOWNTIME) " +
                "from RE_TRANS_UE_YEAR where ASSETYPE='变压器' and CREATETIME like '" + RemindDateUtils.getLastYear() + "%' " +
        ...
        String insertQuery = "INSERT INTO RE_ALL_TRANS_UE_YEAR(ASSETYPE,PRODUCTMODELC,RUNNINGTIME,UTILIZATIONRATIO,DOWNTIME,CREATETIME) VALUES(?,?,?,?,?,?)";
        JDBCOutputFormat.JDBCOutputFormatBuilder outputBuilder =
                JDBCOutputFormat.buildJDBCOutputFormat().setDrivername(DB2_DRIVER_NAME).setDBUrl(DB2_DB_URL)
                        .setQuery(insertQuery).setSqlTypes(type).setUsername(DB2_USERNAME).setPassword(DB2_PASSWORD);
        result.output(outputBuilder.finish());

        env.execute("TotalTransformerUseEfficiencyYear");
    }
