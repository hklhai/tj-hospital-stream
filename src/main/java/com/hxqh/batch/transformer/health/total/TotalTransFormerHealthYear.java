...
public class TotalTransFormerHealthYear {

    public static void main(String[] args) throws Exception {
        final int[] type = getType();

        ExecutionEnvironment env = ExecutionEnvironment.getExecutionEnvironment();
        String quarterQuery = "select PRODUCTMODELC,ASSETYPE,PRODUCTMODEL,LOCATION,avg(SCORE) as SCORE from RE_TRANS_HEALTH_YEAR " +
                "where ASSETYPE='变压器' and CREATETIME ='" + RemindDateUtils.getLastYear() + "' group by PRODUCTMODELC,ASSETYPE,PRODUCTMODEL,LOCATION";
        ...
        String insertQuery = "INSERT INTO  RE_ALL_TRANS_HEA_Y(PRODUCTMODELC,ASSETYPE,PRODUCTMODEL,LOCATION,SCORELEVEL,SCORE,CREATETIME) VALUES(?,?,?,?,?,?,?)";
        JDBCOutputFormat.JDBCOutputFormatBuilder outputBuilder =
                JDBCOutputFormat.buildJDBCOutputFormat().setDrivername(DB2_DRIVER_NAME).setDBUrl(DB2_DB_URL)
                        .setQuery(insertQuery).setSqlTypes(type).setUsername(DB2_USERNAME).setPassword(DB2_PASSWORD);
        sinkRow.output(outputBuilder.finish());

        env.execute("TotalTransFormerHealthYear");
    }
