...
public class TransFormerHealthCompareQuarter {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String quarterQuery = "select IEDNAME,ASSETYPE,LOCATION,PRODUCTMODEL,PRODUCTMODELC,SCORE,CREATETIME from RE_TRANS_HEALTH_QUARTER where ASSETYPE='变压器' and CREATETIME in " + RemindDateUtils.getLastTwoQuarterString();
        ...
        String insertQuery = "INSERT INTO RE_TRANS_HEA_COM_QUARTER (IEDNAME,ASSETYPE,LOCATION,PRODUCTMODEL,PRODUCTMODELC,SCORE,CREATETIME,COMPARISON,RATIO) VALUES(?,?,?,?,?,?,?,?,?)";
        JDBCOutputFormat.JDBCOutputFormatBuilder outputBuilder =
                JDBCOutputFormat.buildJDBCOutputFormat().setDrivername(DB2_DRIVER_NAME).setDBUrl(DB2_DB_URL)
                        .setQuery(insertQuery).setSqlTypes(type).setUsername(DB2_USERNAME).setPassword(DB2_PASSWORD);
        result.output(outputBuilder.finish());

        env.execute("TransFormerHealthCompareQuarter");
    }
