...
public class TransFormerHealthYear {
...
//        String sqlFirst = "select IEDName,assetYpe,location,productModel,productModelC,alarmLevel,count(1) as levelCount from yx1 where Val='1' and assetYpe = '变压器' and ColTime>='2020-03-01 00:00:00' and ColTime<='2020-04-30 23:59:59'  group by IEDName,alarmLevel,assetYpe,location,productModel,productModelC";
        ...
        String insertQuery = "INSERT INTO  RE_TRANS_HEALTH_YEAR(IEDNAME,ASSETYPE,LOCATION,PRODUCTMODEL,PRODUCTMODELC,SCORELEVEL,SCORE,CREATETIME) VALUES(?,?,?,?,?,?,?,?)";
        JDBCOutputFormat.JDBCOutputFormatBuilder outputBuilder =
                JDBCOutputFormat.buildJDBCOutputFormat().setDrivername(DB2_DRIVER_NAME).setDBUrl(DB2_DB_URL)
                        .setQuery(insertQuery).setSqlTypes(type).setUsername(DB2_USERNAME).setPassword(DB2_PASSWORD);
        result.output(outputBuilder.finish());

        env.execute("TransFormerHealthYear");
    }
